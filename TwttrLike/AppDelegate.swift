...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        UINavigationBar.appearance().barTintColor = UIColor(red: 0, green: 172/255, blue: 237/255, alpha: 1)
        UINavigationBar.appearance().tintColor = UIColor.white
        
        if User.currentUser != nil {
            let accessToken = defaults.object(forKey: "UserAccessToken")
            let accessTokenIsValid = TwitterClient.sharedInstance.requestSerializer.accessToken.isExpired
            
            print("Current User: \((User.currentUser?.name)!)")
            print("Access Token: \(accessToken!)")
            print("Access Token Expired: \(accessTokenIsValid)")
